# -*- coding: utf-8 -*-
"""
Created on Thu Nov 19 03:57:46 2020
...
import requests
import time
# implemented http session for retries
...
def read_url_to_html(url, session):
    t0 = time.time()
    try:
        ...
        t1 = time.time()
        print(url, ' took', t1 - t0, ' seconds')

def read_url_to_json(url, session):
    t0 = time.time()
    try:    
...
def get_imdb_data(s, all_data, imdb_data, movie_on_imdb):
...
def get_metacritic_data(s, all_data, session):
    ...
    if mc_search:
        for result in mc_search:
            ...
            if year == all_data["year"]:
                first_link = stats.find("h3", {"class": "product_title basic_stat"})
                first_link = first_link.find("a")
                link = 'http://www.metacritic.com' + first_link["href"]

                s = read_url_to_html(link, session)
...
def get_rotten_tomatoes_data(s, session, all_data):
    
    rt_link = 'N/A'
    if s.get("movies") is not None:
        for i in s["movies"]:
    ...
    if rt_link != 'N/A':
        s = read_url_to_html(rt_link, session)
...
def movie_data(movie_on_imdb, session):
    all_data = {}
    # Start IMDB crawling
    s = read_url_to_html(movie_on_imdb, session)
    ...
    all_data = get_imdb_data(s, all_data, imdb_data, movie_on_imdb)
    name = clean_name(all_data["name"])    
    # Start Meta Critic crawling
    s = read_url_to_html(mc_url % name, session)
    all_data = get_metacritic_data(s, all_data, session)

    # Start Rotten Tomatoes crawling

    s = read_url_to_json(rt_url % name, session)
    all_data = get_rotten_tomatoes_data(s, session, all_data)
...
#print (movie_data("https://www.imdb.com/title/tt0032557", session))